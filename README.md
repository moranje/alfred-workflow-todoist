...
| _filter_   | `"@15min & #Personal"`     | Filter through Todoist. [Read the docs](https://get.todoist.help/hc/en-us/articles/205248842-Filters). _This is a premium feature_.       |
...
| _error_tracking_      | `true` or `false` (default `true`)                                                                                           | This is used to track erros through Sentry.io. When an unexpected error happens in the workflow it is stored in sentry. Extra measures are in place to make sure personal information (like the api token or task details) are not send to Sentry. |
