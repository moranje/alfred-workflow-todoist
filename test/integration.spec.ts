...
describe('Integration:', () => {
  ...
  describe('Create task', () => {
    it('should return a parsed JSON response', async () => {
      expect.assertions(1);
      const query = 'Get milk @15min @at_home #Work p1, tomorrow';
  ...
  describe('Select setting', () => {
    it('should edit token setting', () => {
      Command().verifySetting(
        'token',
        '0123456789abcde0123456789abcde0123456789'
      );
      const items = JSON.parse(spy.mock.calls[0][0]).items;
      const response = JSON.parse(items[0].arg);
    ...
    it('should edit anonymous_statistics setting', () => {
      ...
      const items = JSON.parse(spy.mock.calls[0][0]).items;
      const response = JSON.parse(items[1].arg);
  ...
  describe('Store setting', () => {
    it('should store a token setting back to disk', () => {
      ...
      const response = spy.mock.calls[0][0];
